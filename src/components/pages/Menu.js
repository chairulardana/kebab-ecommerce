    ...
    useEffect(() => {
        ...
        try {
            const [drinksData, kebabsData, paketMakananData, snacksData] =
            await Promise.all([
            ...
            const kebab = kebabsData.data.find(
                (k) => k.id_Kebab === paket.id_Kebab
            );
            const snack = snacksData.data.find(
                (s) => s.id_Snack === paket.id_Snack
            );
            const drink = drinksData.data.find(
                (d) => d.id_Drink === paket.id_Drink
            );
    ...
    const tambahKeCart = async () => {
        ...
        const newItem = {
            ...
            selectedItem.nama_Minuman ||
            selectedItem.nama_Kebab ||
            selectedItem.nama_Snack,
        };
        ...
        if (selectedItem.id_Paket) {
            try {
            await axios.put(
                `http://192.168.1.23:9999/api/PaketMakanan/${selectedItem.id_Paket}`,
                {
                ...selectedItem,
    ...
    const getImage = (item, title) => {
        if (title === "Paket Makanan") {
        return item.image || "/images/default-paket.png";
        }
        if (title === "Kebab") {
        return item.imageUrl || "/images/default-food.png";
        }
    ...
    const renderMenu = (title, items) => (
        ...
        <div className="menu-items">
            {items.map((item) => {
            const imageSrc = getImage(item, title); // Gunakan fungsi getImage yang diperbarui

            return (
                <div
                className="menu-item-custom"
                key={
                    item.id_Drink || item.id_Kebab || item.id_Paket || item.id_Snack
                }
                >
                <div className="image-container">
                    ...
                    />
                    {item.stock === 0 && (
                    <div className="stock-overlay">STOK HABIS</div>
                    )}
                </div>
                <div className="menu-item-body">
                    ...
                    {title === "Paket Makanan"
                        ? item.nama_Paket
                        : item.nama_Minuman || item.nama_Kebab || item.nama_Snack}
                    </h3>
                    {title === "Minuman" && (
                    <>
                        <p>Suhu: {item.suhu}</p>
                    ...
                    <>
                        ...
                        >
                            {formatHarga(item.harga_Paket)}
                        </span>
                        </p>
                        <p>
                        <strong style={{ color: "green", fontSize: "2.1em" }}>
                            Hanya {" "}
                            {formatHarga(item.harga_Paket_After_Diskon)}
                        </strong>
                        </p>
                        <p>Diskon: {item.diskon}%</p>
                        <p>Kebab: {item.nama_Kebab}</p>
                        <p>Snack: {item.nama_Snack}</p>
                        <p>Minuman: {item.nama_Minuman}</p>
                        <p>Hanya Ada {item.stock} Stock </p>
                    </>
                    )}
                    {title === "Snack" && (
                        <>
                             
                            <p>Harga: {formatHarga(item.harga)}</p> 
                            <p>Stok: {item.stock}</p> 
                        </>
                    )}
                    <Button
        ...
        <Modal show={showModal} onHide={() => setShowModal(false)} centered>
            ...
            {selectedItem && (
                ...
                <img
                    src={getImage(
                    selectedItem,
                    selectedItem.id_Paket ? "Paket Makanan" : ""
                    )}
                    alt="menu"
                    className="menu-image-modal"
                    style={{ width: "100%", height: "auto" }}
                />
                <p>
                    Harga:{" "}
                    {formatHarga(
                    selectedItem.harga || selectedItem.harga_Paket_After_Diskon
                    )}
                </p>
                <Form.Group controlId="formJumlah">
                    ...
                    onChange={(e) =>
                        ...
                        Math.max(
                            1,
                            Math.min(selectedItem.stock, Number(e.target.value))
                        )
                        )
                    }
                    />
